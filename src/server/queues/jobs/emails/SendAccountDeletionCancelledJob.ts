...
const log = Logger.getInstance('deletion-cancelled-worker');
...
class SendAccountDeletionCancelledJob extends BaseJob<AccountDeletionCancelledJobData> {
    static jobName = JOB_NAMES.SEND_ACCOUNT_DELETION_CANCELLED;
    ...
    async handle(job: Job<AccountDeletionCancelledJobData>) {
        ...
        log.notice('account deletion cancelled email sent', to);
    }
}

queueManager.registerJob(new SendAccountDeletionCancelledJob().getDefinition());
